...
func set0{
    ...
    %{
        from tayt.workers.fuzzer_worker_exception import AssertionException
        if ids.remainder < 10:
            raise AssertionException(f"MyException with remainder {ids.remainder}")
        else:
            raise AssertionException(f"MyException")
    %}
    if remainder == 0:
        flag0.write(1)
        set_flag0.emit(val)
